...
| 3 | Distinguish dev assets from desired outcome | **HARD** | `~/.claude/hooks/prototype-scaffold-guard.sh` (PreToolUse Write/Edit/MultiEdit) blocks scaffold tokens in `frontend/src`. **Contract-corrected 2026-07-14**: had shipped on the non-denying `{"continue":false}` form (its CI assertion likewise grepped `"continue":false` -- certifying emission, not denial: a Lesson-16 hole); switched to the DENY contract `{"decision":"block"}` (+ `permissionDecision:"deny"`), CI assertion updated. See row 22 |
...
| 22 | An unattended loop without hard stop conditions ends by accident | **HARD** | `hooks/loop-cap-guard.sh` (PreToolUse on scheduling tools: CronCreate / ScheduleWakeup) blocks a launch whose input declares no stop primitive (iteration cap, budget cap, expiry, or escalation path) + CLAUDE.md loop-launch gate. Built 2026-07-05; **live-fire verified + contract-fixed same day**: the first cut emitted `{"continue":false}`, which prints a block message but does NOT deny the tool -- a capless `ScheduleWakeup` still scheduled and fired. Corrected to the PreToolUse DENY contract `{"decision":"block", ...}` (+ `permissionDecision:"deny"`); re-tested live, the capless call is now denied (tool error, no schedule). Same fix applied to `dev-server-guard.sh` + `secret-scanner.sh`, which shared the wrong contract, and the CI assertions now grep `"decision":"block"` (they previously grepped `"continue":false` -- certifying emission, not denial: a Lesson-16 hole inside the enforcement layer). **Follow-up 2026-07-14**: the same-day fix had MISSED `prototype-scaffold-guard.sh` (row 3) -- it still emitted `{"continue":false}` and its scaffold CI assertions still grepped `"continue":false`. Now corrected to the DENY contract + assertions updated (incl. a `permissionDecision:"deny"` assertion), so all four PreToolUse guards share the denying form. |
...
| 25 | Machine translation flattens the markup it can't see; ship both languages in the DOM | SEMI | Standing bilingual-DOM rules: every translatable node ships paired `<span lang="en">…</span><span lang="es">…</span>` with CSS showing the active language; edit ENGLISH-first then re-translate; segment prose around a shared citation anchor (never duplicate an `id`); a card's JSON-island modal title mirrors its face. Verify by render symmetry (in one language, zero elements of the other are visible). Judgment/authoring practice -- no honest HARD; *next gate*: a build check that flags a translatable node present in only one language |
| 26 | The manufactured-contrast aphorism is the loudest slop tell; state the plain thing | SEMI | AI-Slop Design Gate "Copy tells" now names the manufactured-contrast / "N things. One thing" / tacked-on-nominalization family explicitly, with the two source taxonomies (impeccable.style/slop, ignorance.ai field guide); audit BOTH copy and design against the named checklist before shipping, grading each hit deliberate-on-brand vs present-because-default (structural numbered IDs are navigation, keep them; a decorative kicker over every heading is the tell). Judgment-heavy (L16 shape -- can't grade your own reflexes without an external rubric); *next gate*: `evidence-audit.sh` copy-tell family flagging the "X, not Y" / "Not X. Y." two-beat fragment used as a heading/title |
| 27 | An operator-named unvetted skill runs as an instructed-subagent council; subjective edits are shown before they ship | SEMI | Skill Security Gate auto-REJECT for unvetted skills stands (L21 shape -- the request doesn't change what executes); deliver a named-but-unvetted capability through a trusted channel instead: the skills.sh 3-audit, or reproduce the discipline as instructed Opus subagent lenses (built-in WebFetch-as-inert-data replaces a scraper skill). Plus the MOCK-BOTH discipline for any subjective copy/design edit on a live surface: synthesize -> present before->after -> sign-off -> apply -> verify. Ties to row 21; *next gate*: a pre-apply diff-and-sign-off checklist line for taste-owned changes on a live surface |
